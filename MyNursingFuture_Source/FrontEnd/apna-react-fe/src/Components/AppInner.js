  ...
  return {
    onMount: function () {
      ...
      if (cookies.getItem('token')) {
        ...
        const userType = cookies.getItem('_ut');
        if (userType === 'employer') {
          dispatch(fetchCheckEmployerAuth(token))
        } else {
          dispatch(fetchCheckNurseAuth(token));
        }
      }
